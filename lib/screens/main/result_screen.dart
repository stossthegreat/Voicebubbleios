...
class _ResultScreenState extends State<ResultScreen> {
  ...
  /// Show voice recording dialog for instructions
  Future<void> _showInstructionsVoiceRecording(BuildContext context) async {
    ...
    await showDialog(
      ...
      builder: (context) => _VoiceInstructionsDialog(
        onInstructions: (text) {
          instructions = text;
        },
      ),
    ...
    if (instructions != null && instructions!.isNotEmpty) {
      await _addMoreAndRewrite(instructions!);
    }
  }
...
// Voice Instructions Dialog Widget
class _VoiceInstructionsDialog extends StatefulWidget {
  final Function(String) onInstructions;

  const _VoiceInstructionsDialog({
    required this.onInstructions,
  });

  @override
  State<_VoiceInstructionsDialog> createState() => _VoiceInstructionsDialogState();
}

class _VoiceInstructionsDialogState extends State<_VoiceInstructionsDialog> {
  ...
  Future<void> _startRecording() async {
    try {
      ...
      _recordingPath = '${directory.path}/instruction_${DateTime.now().millisecondsSinceEpoch}.m4a';
  ...
  void _submit() {
    if (_transcribedText.isNotEmpty) {
      widget.onInstructions(_transcribedText);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              'Tap to speak your instructions',
              style: TextStyle(
            ...
            // Recording Button
            GestureDetector(
              onTap: _isRecording ? _stopRecordingAndTranscribe : _startRecording,
              child: Container(
                width: 120,
                height: 120,
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: (_isRecording ? Colors.red : const Color(0xFF3B82F6)).withOpacity(0.3),
                      blurRadius: 20,
                      spreadRadius: 5,
                    ),
                  ],
